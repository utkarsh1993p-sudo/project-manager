  ...
  return (
    <header className="h-14 md:h-16 border-b border-gray-100 bg-white px-4 md:px-6 flex items-center justify-between shrink-0">
      <div className="pl-10 md:pl-0">
        ...
        {subtitle && (
          <p className="text-xs md:text-sm text-gray-400 truncate max-w-xs md:max-w-none">
            {subtitle}
          </p>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        <button className="w-8 h-8 md:w-9 md:h-9 rounded-lg border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-50 hover:text-gray-700 transition-all duration-150 cursor-pointer relative">
          <Bell size={15} />
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-blue-600 rounded-full shadow-[0_0_4px_rgba(37,99,235,0.5)]" />
        </button>
      </div>
    </header>
  );
